//! Nvidia API client and Rig integration
//!
//! # Example
//! ```
//! use rig::providers::nvidia;
//!
//! let client = nvidia::Client::new("NVIDIA_API_KEY");
//!
//! let nvidia_chat = client.completion_model(nvidia::NVIDIA_CHAT);
//! ```
...
// Main Nvidia Client
// ================================================================
const NVIDIA_API_BASE_URL: &str = "https://integrate.api.nvidia.com/v1";

#[derive(Debug, Default, Clone, Copy)]
pub struct NvidiaExt;
#[derive(Debug, Default, Clone, Copy)]
pub struct NvidiaExtBuilder;

type NvidiaApiKey = BearerAuth;

impl Provider for NvidiaExt {
    type Builder = NvidiaExtBuilder;
...
impl<H> Capabilities<H> for NvidiaExt {
...
impl DebugExt for NvidiaExt {}

impl ProviderBuilder for NvidiaExtBuilder {
    type Output = NvidiaExt;
    type ApiKey = NvidiaApiKey;

    const BASE_URL: &'static str = NVIDIA_API_BASE_URL;
}

pub type Client<H = reqwest::Client> = client::Client<NvidiaExt, H>;
pub type ClientBuilder<H = reqwest::Client> = client::ClientBuilder<NvidiaExtBuilder, String, H>;

impl ProviderClient for Client {
    type Input = NvidiaApiKey;
...
/// The response shape from the Nvidia API
...
pub enum Message {
    ...
    Assistant {
        ...
        /// only exists on `nvidia-reasoner` model at time of addition
        #[serde(skip_serializing_if = "Option::is_none")]
...
pub(super) struct NvidiaCompletionRequest {
    model: String,
...
impl TryFrom<(&str, CompletionRequest)> for NvidiaCompletionRequest {
    type Error = CompletionError;

    fn try_from((model, req): (&str, CompletionRequest)) -> Result<Self, Self::Error> {
        if req.output_schema.is_some() {
            tracing::warn!("Structured outputs currently not supported for Nvidia");
        }
    ...
    T: HttpClientExt + Clone + Default + std::fmt::Debug + Send + 'static,
{
    ...
    > {
        ...
        let request =
            NvidiaCompletionRequest::try_from((self.model.as_ref(), completion_request))?;

        if enabled!(Level::TRACE) {
            tracing::trace!(target: "rig::completions",
                "Nvidia completion request: {}",
    ...
    > {
        ...
        if enabled!(Level::TRACE) {
            tracing::trace!(target: "rig::completions",
                "Nvidia streaming completion request: {}",
                serde_json::to_string_pretty(&request)?
            );
        }
            ...
            match event_result {
                ...
                Ok(Event::Message(message)) => {
                    ...
                    if let Some(choice) = data.choices.first() {
                        ...
                        // Nvidia-specific reasoning stream
...
// ================================================================
// Nvidia Completion API
// ================================================================
pub const NVIDIA_CHAT: &str = "nvidia-chat";
pub const NVIDIA_REASONER: &str = "nvidia-reasoner";
...
mod tests {
    ...
    #[test]
    fn test_deserialize_nvidia_response() {
        let data = r#"{
            "choices":[{
                "finish_reason": "stop",
                "index": 0,
